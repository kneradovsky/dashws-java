...
public class ITSubscribtionsTest {
	...
	public void receiveLastEvents() throws IOException, InterruptedException, DeploymentException, URISyntaxException {
		...
		given().with().body("{\"auth_token\":\"" + WSClient.AUTH_TOKEN + "\",\"data\":\"a1\"}").post("/data/d1").then()
				.assertThat().statusCode(204);
		given().with().body("{\"auth_token\":\"" + WSClient.AUTH_TOKEN + "\",\"data\":\"a2\"}").post("/data/d2").then()
				.assertThat().statusCode(204);
		// send subscribe
	...
	public void receiveEvents() throws IOException, InterruptedException {
		...
		given().with().body("{\"auth_token\":\"" + WSClient.AUTH_TOKEN + "\",\"data\":\"a1\"}").post("/data/d3").then()
				.assertThat().statusCode(204);
		given().with().body("{\"auth_token\":\"" + WSClient.AUTH_TOKEN + "\",\"data\":\"a2\"}").post("/data/d4").then()
				.assertThat().statusCode(204);
